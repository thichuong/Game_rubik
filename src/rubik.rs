...
fn setup_materials(mut commands: Commands, mut materials: ResMut<Assets<StandardMaterial>>) {
    ...
    let rubik_materials = RubikMaterials {
        white: materials.add(StandardMaterial { base_color: Color::WHITE, perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        yellow: materials.add(StandardMaterial { base_color: Color::srgb(1.0, 0.9, 0.0), perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        red: materials.add(StandardMaterial { base_color: Color::srgb(0.9, 0.1, 0.1), perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        orange: materials.add(StandardMaterial { base_color: Color::srgb(1.0, 0.4, 0.0), perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        green: materials.add(StandardMaterial { base_color: Color::srgb(0.1, 0.7, 0.1), perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        blue: materials.add(StandardMaterial { base_color: Color::srgb(0.1, 0.2, 0.9), perceptual_roughness: 0.1, metallic: 0.1, ..default() }),
        black: materials.add(StandardMaterial { base_color: Color::srgb(0.05, 0.05, 0.05), perceptual_roughness: 0.3, ..default() }),
    };
    ...
    for x in -1..=1 {
        for y in -1..=1 {
            for z in -1..=1 {
                ...
                // Add faces
                if x == 1 { spawn_face(&mut commands, cubie_id, Face::Right, &face_mesh, &materials.red); }
                else if x == -1 { spawn_face(&mut commands, cubie_id, Face::Left, &face_mesh, &materials.orange); }
                if y == 1 { spawn_face(&mut commands, cubie_id, Face::Up, &face_mesh, &materials.white); }
                else if y == -1 { spawn_face(&mut commands, cubie_id, Face::Down, &face_mesh, &materials.yellow); }
                if z == 1 { spawn_face(&mut commands, cubie_id, Face::Front, &face_mesh, &materials.green); }
                else if z == -1 { spawn_face(&mut commands, cubie_id, Face::Back, &face_mesh, &materials.blue); }
            }
        }
    }
}

fn spawn_face(commands: &mut Commands, parent: Entity, face: Face, mesh: &Handle<Mesh>, material: &Handle<StandardMaterial>) {
    let normal = face.normal();
    let translation = normal * 0.501; // Slightly outside the cubie
    let rotation = Quat::from_rotation_arc(Vec3::Z, normal);
    let face_id = commands.spawn((
        Mesh3d(mesh.clone()),
...
fn handle_rotation_queue(
    ...
    cube_root: Query<Entity, With<RubikCube>>,
) {
    if current.is_some() { return; }
    let Ok(root_entity) = cube_root.get_single() else { return };

    if let Some(rotation_move) = queue.0.pop_front() {
        let (axis, angle) = rotation_move.side.get_rotation_info(rotation_move.direction);
        ...
        for (entity, coord) in cubies.iter() {
            if rotation_move.side.is_cubie_at_side(coord.0) {
                commands.entity(pivot_id).add_child(entity);
...
fn animate_rotation(
    ...
    mut pivot_query: Query<(Entity, &mut Transform, &TargetRotation), With<Pivot>>,
    mut cubie_query: Query<(&mut Transform, &mut GridCoord), (With<Cubie>, Without<Pivot>)>,
    cube_root: Query<Entity, With<RubikCube>>,
) {
    let Some(mut current) = current else { return };
    let Ok((pivot_entity, mut pivot_transform, target)) = pivot_query.get_single_mut() else { return };
    let Ok(root_entity) = cube_root.get_single() else { return };
    ...
    if current.timer.finished() {
        ...
        for &cubie_entity in &current.cubies {
            if let Ok((mut transform, mut coord)) = cubie_query.get_mut(cubie_entity) {
                ...
                // Re-attach to root
                commands.entity(cubie_entity).set_parent(root_entity);
            }
        }
        
        commands.entity(pivot_entity).despawn_recursive();
